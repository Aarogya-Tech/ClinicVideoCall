package com.aarogyaforworkers.aarogyaFDC.SubUser
...
class SubUserDBRepository {
    ...
    * **/
    ...
    private val option : Options? = null

    private var isSubUserProfileOptionList = mutableStateOf(mutableListOf(option))

    var subUserProfileOptionList : MutableState<MutableList<Options?>> = isSubUserProfileOptionList

    fun updateOptionList(list : String){
...
//        isSubUserSessionsList.value = list
        ...
        val sessionsList = list.filter { it.sessionId.isNotEmpty() }.sortedBy { session -> sdf.parse("${session.date} ${session.time}") }
